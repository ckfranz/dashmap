...
class Workout {
  ...
  constructor(coords, distance, duration) {
    // Functionality w ES6 (old)
    // this.date = ...
    // this.id = ...

    // Functionality w upcoming JS Features (new)
...
// const run1 = new Running([43, 79], 5.2, 34, 178);
// const cycle1 = new Cycling([43, 79], 27, 95, 523);
...
class App {
  ...
  _getPosition() {
    if (navigator.geolocation)
      // the following ia a regualr function call -> 'this' set to undefined. Use bind to set 'this'
  ...
  _newWorkout(e) {
    ...
    const validInputs = (...inputs) =>
      inputs.every(inp => Number.isFinite(inp)); // using rest parameters '...' gives us an array
    // Helper function:
    ...
    // Note: 'if else' isnt as much any more - also looks cleaner
    ...
    if (type === 'cycling') {
      ...
      if (
        ...
        !allPositive(distance, duration) // Note: elevation can be negative
      )
        return alert('Inputs must be positive numbers!');
